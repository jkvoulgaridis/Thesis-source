...
## Edit core-site.xml to set hdfs default path to hdfs://master:9000
		CORE_SITE_CONTENT="\t<property>\n\t\t<name>fs.default.name</name>\n\t\t<value>hdfs://spark-master:5432</value>\n\t</property>"
...
## Set the two datanodes for the distributed filesystem
		echo "spark-master" > /opt/hadoop/etc/hadoop/workers
		#echo "slave" >> /home/user/hadoop/etc/hadoop/slaves

## Export JAVA_HOME variable for hadoop
		sed -i '/export JAVA\_HOME/c\export JAVA\_HOME=\/usr\/local\/openjdk-11\/' /opt/hadoop/etc/hadoop/hadoop-env.sh
}
